...
// Serializa errores para loguear con metadatos minimos.
...
// Logger contextual por peticion para trazabilidad sencilla.
...
// Registra un usuario mediante join code en una transaccion.
// ======================================================
export const registerUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!joinCode || typeof joinCode !== "string") {
      return res.status(400).json({ error: "Debe proporcionar un codigo de registro valido." })
    }

    const normalizedCode = joinCode.trim().toUpperCase()
    if (!/^[A-Z0-9]{3}-[A-Z0-9]{3}$/.test(normalizedCode)) {
      return res.status(400).json({ error: "Formato de codigo invalido. Usa XXX-YYY." })
    }
    ...
    try {
      ...
      if (!joinCodeRecord) {
        ...
        return res.status(400).json({ error: "Codigo de registro no encontrado." })
      }
      ...
      if (joinCodeRecord.max_uses !== null && joinCodeRecord.used_count >= joinCodeRecord.max_uses) {
        await client.query("ROLLBACK")
      ...
      const message = joinCodeRecord.auto_approve
        ? "Cuenta creada y activada correctamente. Ya puedes iniciar sesion."
        : "Solicitud registrada correctamente. Un administrador debe aprobar tu cuenta antes de iniciar sesion."
...
export const loginUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Crear token JWT y guardar sesion activa como refresh token.
    ...
    try {
      await createRefreshToken(user.id, token)
    } catch (dbErr) {
