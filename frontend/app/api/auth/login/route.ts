  ...
  try {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        console.warn(
          `[auth/login] backend ${res.status} on attempt ${attempt + 1}. Retrying in ${delay}ms. Body: ${bodyPreview}`
        );
      } catch (fetchErr: any) {
        if (attempt === MAX_RETRIES) {
          console.error(`[auth/login] network failure on attempt ${attempt + 1}:`, fetchErr);
        ...
        console.warn(
          `[auth/login] network error on attempt ${attempt + 1}: ${fetchErr?.message ?? fetchErr}. Retrying in ${delay}ms`
        );
      }
